# IP2Location.io Erlang API

## ip2locationio Module

```{py:function} new(APIKey)
Configuration registry. Set the IP2Location.io API key.

:param String APIKey: (Required) The IP2Location.io API key.
```

```{py:function} lookupipgeolocation(IPAddress, Lang)
...
```{py:function} lookupdomainwhois(Domain)
...
```{py:function} lookuphosteddomain(IPAddress, Page)
Retrieve hosted domains information for an IP address.

:param String IPAddress: (Required) The IP address (IPv4 or IPv6).
:param String Page: (Optional) Page of the result.
