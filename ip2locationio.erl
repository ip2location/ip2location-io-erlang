...
new(APIKey) ->
	...
	case ets:info(myconfig) of
		undefined ->
			...
			ets:insert(myconfig, {version, "1.1.1"}),
...
lookupipgeolocation(IPAddress) ->
	lookupipgeolocation(IPAddress, undefined).

lookupipgeolocation(IPAddress, Lang) ->
	ssl:start(),
	inets:start(),
...
getpunycode(Domain) ->
	idna:encode(Domain, [uts46]).

getnormaltext(Domain) ->
	idna:decode(Domain, [uts46]).

checktlds([], _) -> no_match;
checktlds([H|T], Host) ->
	case lists:suffix(H, Host) of
		true ->
			Replacement = string:replace(Host, H, ""),
			Tmp = lists:nth(1, Replacement),
			Replacement2 = string:find(Tmp, ".", trailing),
			...
			Index = string:length(lists:nth(1, string:replace(Tmp, Replacement3, ""))) + X,
			string:slice(Host, Index);
		_ ->
			checktlds(T, Host)
	end.
...
getdomainname(Url) ->
	...
	Parsed = uri_string:parse(Fullurl),
	case Parsed of
		{error, Reason, _} ->
			Reason;
		_ ->
			Host = maps:get(host, Parsed),
			Host2 = getpunycode(Host),
			checktlds(Tlds, Host2)
	end.
